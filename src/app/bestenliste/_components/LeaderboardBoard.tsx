...
export function LeaderboardBoard() {
  ...
  return (
    <>
        ...
        {options.games.map((game) => {
          ...
          return (
            <button
              ...
              className={`min-h-12 shrink-0 rounded-full border px-4 text-sm font-semibold whitespace-nowrap transition-colors ${
                active
      ...
      {data?.viewerEntry && (
        <OwnEntry entry={data.viewerEntry} scoreKind={scoreKind} />
      )}
    </>
  );
}
...
/** The viewer's best entry, pinned in thumb reach above the BottomBar. */
function OwnEntry({
  entry,
  scoreKind,
}: {
  ...
  return (
    <div className="fixed inset-x-0 bottom-[calc(4rem+env(safe-area-inset-bottom)+0.75rem)] z-30 mx-auto w-full max-w-md px-4 md:bottom-6">
      <PixelFrame color="var(--color-neon-cyan)">
        <div className="bg-surface">
          <p className="sr-only">Dein bester Eintrag</p>
          <ol>
            <RankRow
              entry={entry}
              scoreKind={scoreKind}
